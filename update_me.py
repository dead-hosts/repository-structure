...
class Initiate(object):
    ...
    def PyFunceble(self):  # pylint: disable=invalid-name
        ...
        command_to_execute = '%s --dev -u && ' % (tool_path)
        command_to_execute += '%s -v && ' % (tool_path)
        command_to_execute += 'export TRAVIS_BUILD_DIR=%s && ' % environ['TRAVIS_BUILD_DIR']
        command_to_execute += '%s --dev --autosave-minutes %s --commit-autosave-message "[Autosave] %s" --commit-results-message "[Results] %s" -i && ' % (  # pylint: disable=line-too-long
        ...
        command_to_execute += '%s --travis -a -ex --plain --split -t 2 -f %s ' % (
